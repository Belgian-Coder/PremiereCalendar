...
11. The UI still applies `PremiereFilter` in memory for local-only filters and final consistency, then renders one selected day. Moderate days render 10 cards at a time, expose show-more controls, and use a scroll sentinel to auto-load the next batch while browsing. Days over 40 items switch to .NET 11 Blazor `Virtualize` over two-card rows so large days do not keep every card mounted in the DOM. The `/series` and `/movies` routes lock the media type before filtering; the combined `/` route keeps the media checklist.
...
On desktop, the week renders as one selected day inside a max-width `1920px` calendar. The sticky day bar is the navigation surface: clicking a day updates the mounted day component and scrolls the board back to the top. Cards are laid out two per row when the viewport has room. Moderate days start with 10 cards and continue loading in 10-card batches as the scroll sentinel approaches the viewport; very dense days switch to .NET 11 Blazor `Virtualize` so only visible and overscan rows stay mounted. The component provides an initial virtualized row size for first render accuracy, while .NET 11 adapts to measured variable-height rows at runtime.
