...
7. Each accepted item is normalized into a `PremiereItem` with a stable canonical ID: `tv:{tmdbId}` for series premieres, `tv:{tmdbId}:air:{yyyyMMdd}` or `tv:{tmdbId}:s{season}e{episode}` for series episodes, and `movie:{tmdbId}` for movies. The item also receives date semantics, merge contributions, and missing-data diagnostics that are persisted in the week cache.
...
Open a card's provenance details when a row looks surprising. The inspector shows which provider contributions were accepted, which IDs matched, which date source was chosen, and why fields such as IMDb, TVDB, Rotten Tomatoes, poster, trailer, or source metadata are still missing.
...
Settings maintenance actions can repair cached weeks without rebuilding unrelated weeks. Backfill scores updates cached cards with IMDb dataset ratings plus OMDb Rotten Tomatoes, audience, critic, and Metacritic values when exact IDs are available. Repair IDs uses TMDb details to fill missing IMDb, TVDB, and Wikidata IDs and leaves uncertain items unchanged.
