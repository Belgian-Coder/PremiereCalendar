...
8. IMDb scores and vote counts come from the configured database import of IMDb's ratings dataset. The gzip input is streamed line-by-line into one transaction, and visible-card lookups are grouped into bounded database batches. `OmdbClient` optionally enriches Rotten Tomatoes, Metacritic, plot, and poster artwork by exact IMDb ID. Missing and zero scores are omitted from the card. Score backfill can later hydrate cached cards when IMDb or OMDb data arrives after the week cache was written.
