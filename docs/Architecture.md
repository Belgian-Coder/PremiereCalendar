...
8. IMDb scores and vote counts come from the local SQLite import of IMDb's ratings dataset. `OmdbClient` optionally enriches Rotten Tomatoes, Metacritic, plot, and poster artwork by exact IMDb ID. OMDb failures or missing ratings return `n/a` behavior instead of breaking the page. Score backfill can later hydrate cached cards when IMDb or OMDb data arrives after the week cache was written.
