...
3. Optional `IPremiereDiscoveryProvider` implementations add external candidate rows from Trakt calendars and series-only TVmaze schedules. Providers that implement `IStreamingPremiereDiscoveryProvider` can report candidate batches as their own API calls complete. Candidates that map to TMDb become verified cards; candidates that cannot map yet are retained as unverified cards below the verified results.
...
8. IMDb scores and vote counts come from the local SQLite import of IMDb's ratings dataset. The gzip input is streamed line-by-line into one transaction, and visible-card lookups are grouped into bounded SQLite batches. `OmdbClient` optionally enriches Rotten Tomatoes, Metacritic, plot, and poster artwork by exact IMDb ID. OMDb failures or missing ratings return `n/a` behavior instead of breaking the page. Score backfill can later hydrate cached cards when IMDb or OMDb data arrives after the week cache was written.
...
12. `PremiereCard` routes poster and backdrop image URLs through `/cached-image`, which stores allowed remote image bytes under the local image cache. Poster cards request `w=185`; browsers that advertise WebP receive a width-specific WebP representation, while other clients retain the JPEG/original fallback. Format participates in the cache key and the response varies on `Accept`. Remote image downloads and decodes have separate server-side concurrency caps. The card renders a tiny placeholder first and moves the real URL into `data-lazy-src`; a browser intersection observer fills `src` only when the card approaches the viewport.
...
14. `WeekDiagnosticsService` records source counts, mapped/unmapped provider candidates, score coverage, missing-ID rates, language distribution, and anomaly flags for the loaded week under `Diagnostics.Week.*` app-state keys.
15. `AdjacentWeekPrefetcher` warms nearby week caches after the visible week loads, using the same saved filters as the visible request. `CurrentWeekCalendarWarmupService` also runs on startup and at a configurable wake interval, but routine warmup uses `forceRefresh: false` and warms stale or missing full-week media cache keys that the UI can reuse. The priority starts with today/tomorrow/yesterday/rest-of-week and expands to the rest of the month, adjacent months, and months +2 through +6. Each wake checks cache metadata first and only starts a bounded number of missing or stale windows, while cycle and per-window budgets keep background work from running indefinitely.
16. SIMKL's public calendar payloads are streamed and filtered to the requested date range before retention. A private eight-entry LRU bounds those range snapshots so global TV/movie payloads cannot accumulate in the process-wide memory cache.
17. `CalendarDay` mounts at most 40 rich cards. Bottom and top intersection sentinels move a ten-card window in either direction; measured spacers preserve the user's scroll position without relying on a fixed card-height estimate.
...
TMDb is the canonical identity source. TMDb discovery always uses the selected date window and technical safety parameters. When filters are saved, the app also sends TMDb-supported filters such as media type, language, genres, watch providers, monetization types, TMDb vote filters, runtime, keyword IDs, movie release types, movie certifications, and selected TV network IDs. Trakt can add movie or series candidates, while TVmaze schedule discovery can add series candidates only. Mapped candidates become verified TMDb-backed cards; unmapped candidates are retained as unverified external cards and retried on future refreshes. Watchmode is used only as a streaming availability fallback. SIMKL is account/library sync state, not a calendar discovery provider.
...
Strict title/year matching is used only as a bounded fallback after external-ID lookup fails. Ambiguous matches remain unverified.
...
The calendar writes the active week and non-default filter state to query parameters. A copied URL restores the same week, selected day, media type selection on `/` when it is not the default All view, sort order when it changes, per-media language selections, origin countries, genres, selected provider/channel sources, watch countries, availability types, certifications, runtime windows, and keyword text. Default values such as date sorting, ascending direction, all languages, all origins, score `0-10`, minimum votes `0`, and runtime `0-360` are intentionally omitted from generated URLs. Multi-language values are serialized as comma-separated route-specific parameters such as `seriesLang=en,nl` and `movieLang=fr,nl`. The series row-scope control is app-specific and maps to TMDb `air_date` versus `first_air_date` requests. On `/series` and `/movies`, the route itself is the media type. The browser stores the last saved non-default filter query separately for `/`, `/series`, and `/movies`, without the week, so each route restores only its own saved filters.

The Actions palette can copy that canonical URL and export the currently visible, already-filtered items as an all-day ICS calendar, CSV, or JSON. Export formatting is deterministic and side-effect-free in `CalendarExportService`; the browser creates the download locally so the server does not need to reconstruct circuit-local filter state.

View sync is opt-in per browser. Each browser gets a local device ID and a friendly name, then joins one SQLite-backed sync group. Calendar routes publish only relative `/`, `/series`, and `/movies` URLs. The latest synced URL is stored separately for All, Series, and Movies, so a Series page follows only Series updates. Active grouped circuits receive live in-process updates and navigate to newer same-route revisions while ignoring self-originated updates, invalid URLs, and non-calendar routes.
...
- `CalendarFilterDialog.razor` owns draft filter state while the pane is open. Cancel/close discards the draft, and Save sends one normalized `CalendarFilters` value back to the page. On mobile it shows a compact active-filter review inside the drawer instead of adding noisy chips to the calendar header.
...
- `CalendarDay.razor` renders empty-day states and a measured two-way window of at most 40 cards. It has a render fingerprint so unchanged day inputs skip rerendering.
...
Each cached `PremiereItem` carries a small diagnostics envelope:

- `DateSemantics` records the chosen date, the source kind, confidence, and a human-readable reason. New-series cards prefer season 1 episode 1 evidence when it is available and consistent; otherwise they fall back to TMDb first-air-date with a lower confidence note. Movie cards distinguish TMDb release dates, primary release dates, and external provider dates.
- `MergeContributions` records which providers contributed to the card, which IDs were matched, the match method, and whether the contribution was accepted or dropped.
- `MissingDataIssues` records missing IMDb ID, TVDB ID, Rotten Tomatoes, IMDb, poster, trailer, or source metadata reasons so missing fields are explainable instead of silent.

The card stays quiet until the provenance details are opened. Week diagnostics stay integrated with existing source details and Settings diagnostics rather than introducing a separate diagnostics route. Anomalies currently cover low item counts, language skew, source contribution drops, high missing-score or missing-ID rates, unmapped external candidates, and source failures.
...
Navigation is a top bar so the calendar can use the full browser width. It exposes combined, series-only, movie-only, and About routes plus a lightbulb theme toggle. On desktop, the selected day board is capped to a `1920px` calendar width, with cards laid out at most two per row. The day selector stays sticky above the board, highlights the selected day, and has a compact filter icon at the left edge. Left Arrow and Right Arrow move between days unless the user is typing in a field. Filters open in a front-of-screen pane with draft state; close/cancel/swipe discards changes, and Save applies them to the calendar and URL. Active filters are summarized by a count badge on the filter button, with clearing handled inside the pane. Source diagnostics stay compact until Source details is opened, where source buttons can scope the visible calendar to one loaded source batch. On mobile, the day selector remains sticky while the selected day and cards use the full available screen width.

## Known Limits

TMDb `air_date`, `first_air_date`, and `primary_release_date` are metadata dates, not Belgian streaming availability dates. TMDb air-date discovery identifies shows airing on a day but does not expose exact episode titles; exact season/episode labels appear only when an external schedule source supplies them. TVmaze schedules improve broadcast/web premiere coverage, but still only become cards when they map back to TMDb. IMDb dataset import is local after it completes; OMDb enrichment is optional and may not return Rotten Tomatoes or Metacritic scores for every title. Diagnostics can explain those gaps, but they do not override TMDb as canonical identity unless an external candidate maps back to a verified TMDb row.
